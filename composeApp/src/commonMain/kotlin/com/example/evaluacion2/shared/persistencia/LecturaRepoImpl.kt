...
class LecturaRepoImpl(
    ...
    override fun listarPorMedidorMes(idMedidor: String, anio: Int, mes: Int) =
        persistenciaDatos.list("lectura_${idMedidor}_${anio}_${mes}")
            .mapNotNull {
                k -> persistenciaDatos.read(k)?.decodeToString()?.split(";")
                ?.takeIf { it.size == 4 }
                ?.let { p ->
                    LecturaConsumo(p[0], p[1].toInt(), p[2].toInt(), p[3].toDouble())
            }
    }

    override fun ultimaLectura(idMedidor: String): LecturaConsumo? {

        val all = persistenciaDatos.list("lectura_${idMedidor}_")

        val lastKey = all.maxByOrNull {
            k -> k.substringAfterLast("_")
            .toIntOrNull() ?: 0
        } ?: return null
        ...
        return persistenciaDatos.read(lastKey)
            ...
            ?.takeIf { it.size == 4 }
            ?.let { p -> LecturaConsumo(p[0], p[1].toInt(), p[2].toInt(), p[3].toDouble()
            ) }
    }
